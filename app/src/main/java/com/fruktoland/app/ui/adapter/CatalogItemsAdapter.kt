...
@Singleton
class CatalogItemsAdapter @Inject constructor(
    var interactor: ModuleInteractor
) : RecyclerView.Adapter<CatalogItemsAdapter.CatalogItemViewHolder>() {

    private val picasso = Picasso.get()
    var catalogItemsList: List<CatalogItem> = emptyList()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CatalogItemViewHolder {
        val itemView = LayoutInflater
            .from(parent.context)
            .inflate(R.layout.catalog_item_recyclerview_item, parent, false)
        return CatalogItemViewHolder(itemView)
    }

    override fun onBindViewHolder(holderItem: CatalogItemViewHolder, position: Int) {
        holderItem.bind(catalogItemsList[position])
    }
